...
public:
    ...
    List<char *> extractLinks(char *fileName, char *currentURL)
    {
        ...
        while (in.get(c))
        {
            ...
            else if (startsWith[index] == '\0')
            {
                ...
                if (isHtml(url) && !isStartsWith(url, "https"))
                {
                    ...
                    if (url[0] == '#')
                    {
                        my_strcat(fullUrl, (char *)"/");
                    }
                    my_strcat(fullUrl, url);
                    myList.addAtLast(fullUrl);
                }
                index = 0;
            }
            else
    ...
    void DFS(char *currentURL, int depthLeft)
    {
        ...
        cout<<"After download\n";
        List<char *> links = extractLinks(downloaded_Path, currentURL);
        // links.display();
        cout<<"Before DFS\n";
